...
filepath = r'E:\30nm实验数据汇总\2022.11.18.duoKjisuan\数据处理\temp/'#使用的临时文件的bash的路径
    ...
    with open(filename, 'r') as file:
        for i in range(kaishi,jieshu,timestep):
            ...
            for t in range(0,chunkshumu,1):#跳过每次读取数据时的头一行后，循环十次，读取其中的内容
                line = file.readline()
                df = pd.read_table(StringIO(line),sep='\s+',names=['Row', 'c_tem[1]'])
            ...
            print(i)
        #输出所有的分块数据
        #yingli_shuchu.to_csv( filename +'.csv')
        ...
        #输出所有的转置后的分块数据
        #yingli_zhuanzhi.to_csv(filename +'转制'+'.csv')
        print(yingli_shuchu)  
        return yingli_shuchu,yingli_zhuanzhi


for folders in filepath:
    ...
    for folder in folders:
        ...
        for file in files:
            ...
            data = chunk_stress(filepath,filen,before,jieshu,timestep,chunknum,tixidaxiao,add,sub,mag)
            data[0].to_csv(folder+file+'正向输出'+'.csv')
            data[1].to_csv(folder+file+'转制输出'+'.csv')
